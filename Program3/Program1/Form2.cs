...
namespace Program1
{
    public partial class Form2 : Form
    {
        ...
        private void pictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            double x = (e.X - W1) / dx;
            double y = (H1 - e.Y) / dy;
            string text = "Точка экстремума: 0 (min)";
            label2.Text = text;
        ...
        public double F1(double x)
        {
            if (x < -1 || x > 1)
            {
                return (Math.Pow(x, 2) * Math.Sqrt(x * x - 1) / (2 * Math.Pow(x, 2) - 1));
            }
            return 0;
        ...
        public double F21(double x)
        {
            if (x > 0)
            {
                if (Math.Abs(x) < 1e-10 || Math.Abs(2 * x * x - 1) < 1e-10 || x * x - 1 < 0)
                {
                    if (x >= -1)
                    {
                        return 1e3;
                ...
                double numerator = (2 * x * Math.Sqrt(x * x - 1) + (x * x * x) / Math.Sqrt(x * x - 1)) * (2 * x * x - 1)
                                   - x * x * Math.Sqrt(x * x - 1) * 4 * x;

                // Знаменатель производной
                double denominator = Math.Pow(2 * x * x - 1, 2);
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            float ixPrev1 = minx, iyPrev1 = (float)F21(ixPrev1);              // координаты предыдущей точки функции f'
            float x2, y2;
